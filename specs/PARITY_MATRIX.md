...
- ✓ 6 · ✗ 1 · ? 6 (dari 13) — run #3: butir 5 ✗→✓ (v0.91.0)
- Prioritas run berikutnya (skor §Fase 1): butir 2 (hint permanen vs `Ready` — keputusan sadar berkomentar di render.rs; run-nya MEMUTUSKAN: sengaja-beda + alasan di matriks, atau selaraskan; skor 3×2÷1=6) → lalu garap butir `?` yang murah buktinya: 13 (capture palette CF) dan 9 (spinner EP).
