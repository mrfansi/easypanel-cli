...
- [tui] Kalau menambah pesan/affordance yang bergantung state per-layar (marks, filter, seleksi), maka audit SEMUA transisi navigasi yang tidak membersihkan state itu (tab produk, drill-in, picker) — pesan yang terbawa ke layar lain menjanjikan aksi (`Esc`/`Space`) yang di sana berbuat hal berbeda. Ditemukan kritik run #2: `cf_set_product` tidak membersihkan `cf.marked`. (asal: run #2)
- [paritas] Kalau EP sudah punya helper presentasi bersama (count_title, dst.), maka CF wajib MEMAKAI helper itu — ekstrak body workspace-agnostic dan delegasikan — bukan meniru formatnya dengan format! sendiri; drift jadi mustahil by construction. Run #3 menemukan 4 judul CF hand-rolled `(X of Y)`. (asal: run #3)
- [tui] Kalau menulis render test layar CF, maka isi `cf.accounts` (bukan hanya `cf.active`) — `cf_empty()` membaca Vec accounts dan menggambar placeholder "No Cloudflare account yet" kalau kosong. (asal: run #3)
- [tui] Kalau status bar CF punya hint permanen, maka hint wajib kalah prioritas dari filter, marks, spinner, error, dan feedback aksi non-error — hint boleh membantu discoverability, tapi tidak boleh menutup bukti bahwa aksi berjalan atau gagal. (asal: run #4)
