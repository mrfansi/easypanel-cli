        ...
        Some(Command::Service(c)) => {
            ...
            match c {
                ServiceCmd::Deploy {
                    ...
                    service_type,
                    force,
                } => commands::service_action(&client, &project, &service, &service_type, "deploy", force),
                ServiceCmd::Restart {
                    project,
                    service,
                    service_type,
                } => commands::service_action(&client, &project, &service, &service_type, "restart", false),
                ServiceCmd::Start {
                    project,
                    service,
                    service_type,
                } => commands::service_action(&client, &project, &service, &service_type, "start", false),
                ServiceCmd::Stop {
                    project,
                    service,
                    service_type,
                } => commands::service_action(&client, &project, &service, &service_type, "stop", false),
                ServiceCmd::Logs {
                    project,
                    service,
                    limit,
